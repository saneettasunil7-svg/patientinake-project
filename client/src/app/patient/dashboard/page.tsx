    ...
    const refreshDoctors = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setDoctors(docs);
                // Extract unique specializations, trimmed and Title Cased for clean UI display
                const depts = Array.from(new Set(docs.map((d: any) => 
                    (d.specialization || 'General Practice').trim().toLowerCase()
                )));
                // Map back to a readable format for the UI while keeping mapping stable
                const formattedDepts = depts.map((d: any) => {
                    const str = String(d);
                    if (str === 'general') return 'General Practice';
...
Generated by City Hospital Telemedicine System
    ...
    const handleBookAppointment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                const doctorId = newAppt.doctor_id;
                setNewAppt({ doctor_id: '', date: '', notes: '' });
                router.push(`/patient/doctor/${doctorId}?autoJoin=true`);
            } else {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-20 items-center">
                        <div className="flex items-center space-x-3">
                            ...
                            <span className="text-xl font-bold tracking-tight text-slate-900">Medi<span className="text-sky-600">Connect</span></span>
                        </div>
                        <div className="flex items-center space-x-4">
                            <button
                                onClick={handleAmbulanceCall}
                                className="hidden sm:flex items-center space-x-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-full text-sm font-bold shadow-lg shadow-amber-500/20 transition-all active:scale-95"
                            >
                                <Ambulance size={18} className="animate-bounce" />
                                <span>Call Ambulance</span>
                            </button>
                            <button
                                onClick={handleEmergencyCall}
                                disabled={isEmergencyLoading}
                                className="hidden sm:flex items-center space-x-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-full text-sm font-bold shadow-lg shadow-red-500/20 transition-all active:scale-95 disabled:opacity-50"
                            >
                                <AlertCircle size={18} className="animate-pulse" />
                                <span>SOS Emergency</span>
                            </button>
                            <NotificationBell />
                            <div className="hidden md:flex items-center space-x-2 text-sm text-slate-600 bg-slate-100 px-4 py-2 rounded-full border border-slate-200">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                                        <div className="relative flex-1 max-w-lg">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all outline-none shadow-sm"
                                            />
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <select
                                                ...
                                                className="px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all outline-none shadow-sm"
                                            >
